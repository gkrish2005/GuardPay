...
import { prisma, razorpay, requestPayment, requestConsent, confirmConsent, createTransactionRequest } from "./gateway.js";
...
console.log("[TEST INITIALIZATION] Starting in-process test server on port 3007...");
import "./index.js";
...
async function main() {
  ...
  // ----------------------------------------------------
  // Test Case 1: 10% Discount ALLOWs (within 15% cap limit)
  // ----------------------------------------------------
  console.log("\n[TEST CASE 1] Running 10% Discount ALLOWs Test...");
  ordersCreateCallCount = 0;
  const cartId1 = `cart_discount_c1_${Date.now()}`;

  // Apply SAVE10 campaign code to cart (write time validation)
  ...
  // Request consent for prod_5 (price = ₹800 -> 10% discount -> ₹720 / 72,000 paise)
  ...
  // Confirm Consent, create transaction, and evaluate
  ...
  // Request consent for prod_5 (price = ₹800 -> 80% discount -> ₹160 / 16,000 paise)
  ...
  // Attempt to request consent for prod_2 (Vaporfly 3 - not in allowedProductIds list)
  ...
  // Verify database and spy safety state
  ...
  // ----------------------------------------------------
  // Test Case 4: Nonexistent Campaign Code Rejection (at write time)
  // ----------------------------------------------------
  ...
  // ----------------------------------------------------
  // Test Case 5: Expired Campaign Rejection (at write time)
  // ----------------------------------------------------
  ...
  // ----------------------------------------------------
  // Test Case 6: Inactive Campaign Rejection (at write time)
  // ----------------------------------------------------
