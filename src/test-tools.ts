...
async function runTests() {
  ...
  if (existingPolicy) {
    if (
      existingPolicy.maxAmount !== 2000000 ||
      existingPolicy.approvalThreshold !== 1000000 ||
      existingPolicy.dailyValueLimit !== 10000000
    ) {
        ...
        items: [
          {
            ...
            price: 50, // Changed price to ₹50 without recomputing hash!
            qty: 1,
          },
        ],
      },
  ...
  // Create a clean consent for a ₹6,500 product
  ...
  // Assert spy calls for ALLOW is exactly 1
  console.log(`- Asserting razorpay.orders.create call count for ALLOW (got: ${ordersCreateCallCount})`);
  ...
  // Create consent for a ₹12,000 product
  ...
  // Assert spy calls for NEEDS_APPROVAL is exactly 0
  console.log(`- Asserting razorpay.orders.create call count for NEEDS_APPROVAL (got: ${ordersCreateCallCount})`);
  ...
  // 7. Test Payment Requests for BLOCK verdict (exceeding policy maxAmount, e.g. ₹20,000)
  console.log("\n7. Testing BLOCK verdict (e.g. ₹24,000 via a reduced policy limit)...");
  // Temporarily reduce policy limits to force a block (write new version)
  const blockPolicy = await prisma.policy.create({
    data: {
      agentId: policy.agentId,
      actionType: policy.actionType,
      maxAmount: 1000000, // Reduced maxAmount to ₹10,000 (1,000,000 paise)
  ...
  // Re-use approvalTxRequest (₹12,000), which now exceeds the new limit of ₹10,000
  ...
  // Assert spy calls for BLOCK is exactly 0
  console.log(`- Asserting razorpay.orders.create call count for BLOCK (got: ${ordersCreateCallCount})`);
  if (ordersCreateCallCount !== 0) {
    throw new Error(`Failed: Expected Razorpay orders.create to not be called, got ${ordersCreateCallCount}`);
  }

  // Restore the policy back to original config (write version increment)
  await prisma.policy.create({
    data: {
      agentId: policy.agentId,
      actionType: policy.actionType,
      maxAmount: 2000000, // Restore back to ₹20,000
      approvalThreshold: policy.approvalThreshold,
      dailyTxLimit: policy.dailyTxLimit,
      dailyValueLimit: policy.dailyValueLimit,
      version: blockPolicy.version + 1,
    },
  });
