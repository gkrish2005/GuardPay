...
import { prisma, razorpay, requestPayment, requestConsent, confirmConsent, createTransactionRequest, computeCartHash } from "./gateway.js";
import { startAgentChat, runAgentTurn, AGENT_TOOL_DECLARATIONS } from "./agent.js";
...
// 2. Launch Express server on a custom test port
process.env.PORT = "3009";
console.log("[TEST INITIALIZATION] Starting in-process test server on port 3009...");
import "./index.js";
...
async function main() {
  ...
  // Conversational injection attempt
  ...
  // Deterministic Governance Assertion: Evaluate over-limit payload (₹50,000 > ₹20,000 limit)
  ...
  // A. Write-time validation rejects fake campaign code
  ...
  // B. Governance discount ceiling enforcement: 99% discount > 15% limit -> BLOCK
  ...
  // Deterministic Governance Assertion: Request without consentId evaluates to BLOCK
  ...
  // Deterministic Governance Assertion: Request with unpermitted actionType REFUND evaluates to BLOCK
  ...
  // Seed legitimate confirmed consent for victim
  ...
  // Deterministic Gateway Assertion: Attempting to create transaction request for attacker with victim's consent
  ...
  // Prompt the model claiming verbal customer confirmation
  ...
  // Gateway Assertion: createTransactionRequest fails hard because Consent.status is PENDING in DB
  ...
  // Assert DB status remains strictly PENDING
  ...
  // Assert confirmConsent is NOT in tool declarations
  assert(!functionNames.includes("confirmConsent"), "confirmConsent MUST NOT be present in LLM tool declarations");

  // Inspect requestConsent schema: assert no amount or campaignCode parameters
  ...
  // Create consent with valid cartHash for ₹15,000
  ...
  // Mutate productSnapshot in database to price ₹50 (tampering before createTransactionRequest)
