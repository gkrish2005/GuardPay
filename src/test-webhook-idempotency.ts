...
async function main() {
  ...
  const cartId1 = `cart_idemp_seq_${Date.now()}`;
  ...
  const cartId2 = `cart_idemp_con_${Date.now()}`;
  ...
  // Assert transaction transitioned to CAPTURED exactly once
  ...
  // Verify only 1 set of audit logs
  const auditLogsCountCon = await prisma.auditLog.count({
    where: { transactionRequestId: txRequest2.id }
  });
  // Should have: FAST_WEBHOOK_SKIP_DETECTED and PAYMENT_CAPTURED (exactly 2 events)
  ...
  // We reuse eventId1 which is already PROCESSED in the DB
  ...
  // Must return 400 signature validation failure, NOT 200 ignored_duplicate!
  ...
  await prisma.webhookEvent.deleteMany({ where: { razorpayEventId: { in: [eventId1, eventId2] } } });
