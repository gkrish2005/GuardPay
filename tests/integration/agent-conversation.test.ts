...
import { prisma, confirmConsent } from "../../src/gateway.js";
import * as gateway from "../../src/gateway.js";
import { startAgentChat, runAgentTurn, AgentContext } from "../../src/agent.js";
...
async function runTestAgent() {
  ...
  if (apiKey) {
    ...
    const handlers = {
      ...
      proposeUpsell: (cId: string) => {
        console.log(`[AGENT TOOL CALL] Executing function "proposeUpsell" with args: {"cartId":"${cId}"}`);
        const res = gateway.proposeUpsell(cId);
        console.log(`[AGENT TOOL RESPONSE] "proposeUpsell" output:`, JSON.stringify(res));
        return res;
      },
      requestConsent: async (cId: string, productId: string, description: string) => {
        console.log(`[AGENT TOOL CALL] Executing function "requestConsent" with args: {"cartId":"${cId}","productId":"${productId}","description":"${description}"}`);
        const res = await gateway.requestConsent(customerId, cId, productId, description);
        console.log(`[AGENT TOOL RESPONSE] "requestConsent" output:`, JSON.stringify(res));
        return res;
      },
      createTransactionRequest: async (cId: string, consentId: string) => {
        console.log(`[AGENT TOOL CALL] Executing function "createTransactionRequest" with args: {"cartId":"${cId}","consentId":"${consentId}"}`);
